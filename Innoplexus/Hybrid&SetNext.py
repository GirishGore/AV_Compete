...
for i in range(0,len(a)):
    for j in range(0,len(a)):
        if(a[i][j] > 0.55 and i != j and documents.iloc[i]['set'] == documents.iloc[j]['set'] and documents.iloc[i]['pub_date'] > documents.iloc[j]['pub_date']):
            #print(documents.iloc[i]['pmid'] , "   " , documents.iloc[j]['pmid'] , "   " , a[i][j])
            print( documents.iloc[i]['set'], "==SET==", documents.iloc[j]['set'])
...
submission.to_csv("E:\\Work\\AV_Compete\\Innoplexus\\HybridAuthDate234055.csv" , index=False)
