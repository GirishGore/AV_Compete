...
for i in range(0,len(a)):
    for j in range(0,len(a)):
        if(a[i][j] > 0.23 and i != j and documents.iloc[i]['set'] == documents.iloc[j]['set'] and documents.iloc[i]['pub_date'] > documents.iloc[j]['pub_date']):
...
for i in range(0,len(a)):
    for j in range(0,len(a)):
        if(a[i][j] > 0.40 and i != j and documents.iloc[i]['set'] == documents.iloc[j]['set'] and documents.iloc[i]['pub_date'] > documents.iloc[j]['pub_date']):
            #print(documents.iloc[i]['pmid'] , "   " , documents.iloc[j]['pmid'] , "   " , a[i][j])
            print( documents.iloc[i]['set'], "==SET==", documents.iloc[j]['set'])
...
from sklearn.feature_extraction.text import TfidfVectorizer
TfidfVec = TfidfVectorizer(binary=True , min_df=3)
def cos_similarity_basic(textlist):
    tfidf = TfidfVec.fit_transform(textlist)
    return (tfidf * tfidf.T).toarray()
documents = info_test
documents['author_str'] = documents['author_str'].fillna("")
a = cos_similarity_basic(documents['author_str'])

from collections import defaultdict
dicts_author = defaultdict(list)

for i in range(0,len(a)):
    for j in range(0,len(a)):
        if(a[i][j] > 0.50 and i != j and documents.iloc[i]['set'] == documents.iloc[j]['set'] and documents.iloc[i]['pub_date'] > documents.iloc[j]['pub_date']):
            #print(documents.iloc[i]['pmid'] , "   " , documents.iloc[j]['pmid'] , "   " , a[i][j])
            print( documents.iloc[i]['set'], "==SET==", documents.iloc[j]['set'])
            dicts_author[documents.iloc[i]['pmid']].append(documents.iloc[j]['pmid'])
...
submission.to_csv("E:\\Work\\AV_Compete\\Innoplexus\\HybridMin2Maxp75_234050_AuthBinSubTM_AuthMin3.csv" , index=False)
